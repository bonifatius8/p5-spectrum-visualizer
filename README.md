...
**課題:** リロード後、マイク許可が再要求されず音声入力が無効化。  
**解決:** AudioContext 初期化をユーザークリックのトリガーに変更。

### 2025-05-09: 可視化・物理シミュレーション

**課題:** エリプスの挙動が単純で視覚的に面白みがない。  
**解決:** 質量・速度・加速度モデル導入。低周波ほど跳ね返り係数が大きくなるよう動的調整。速度比例の抵抗力追加。

### 2025-05-10: クリック範囲修正

**課題:** 「開始」クリックが特定エリアでのみ有効。  
**解決:** canvas 全体にイベントリスナーを設定。

### 2025-05-12: 同心円描画改善

**課題:** 周波数割り当てが bands 幅と不一致。  
**解決:** 円ごとの周波数割り当てを bands 幅に対応。A特性フィルター適用。描画と計算を分離。

### 2025-05-12: モジュール分離

**課題:** `sketch.ts` に物理・FFT ロジックが混在。  
**解決:** `ellipsePhysics.ts`・`spectrumAnalyzerClass.ts` に分離。
