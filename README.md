...
- `VisualLayer` インターフェースで各レイヤーを疎結合。`sketch.ts` の `layers[]` 配列への追加・削除のみで構成変更可能
- 毎フレーム `AudioData` スナップショットを全レイヤーに渡す設計。レイヤーは `AudioContext` に直接アクセスしない
- メインキャンバスは WEBGL、各レイヤーは P2D オフスクリーンバッファで描画し `texture()` + `plane()` でコンポジット
- A特性フィルター + 移動平均でスペクトラムを補正・平滑化
- `AudioContext` 初期化をクリックトリガーに限定（ブラウザ autoplay 制限対応）
